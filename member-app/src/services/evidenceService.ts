...
export async function uploadEvidence(
  ...
  const { data: signed } = await api.post<{
    ...
    token: string;
    storagePath: string;
  }>(`/incidents/${complaintId}/evidence/upload-url`, {
    fileName: att.name,
    type: att.type,
  });
  ...
  const { data } = await api.post<EvidenceItem>(`/incidents/${complaintId}/evidence`, {
...
export async function getEvidence(complaintId: string): Promise<EvidenceItem[]> {
  const { data } = await api.get<EvidenceItem[]>(`/incidents/${complaintId}/evidence`);
  return data;
}
