...
export default function HelpScreen() {
  ...
  const orgPhone = (profile?.organization as { phone?: string } | undefined)?.phone
    ?? (profile?.college as { phone?: string } | undefined)?.phone;
  ...
  return (
    <Screen padded>
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.content}>
        ...
        {(profile?.organization?.name || profile?.college?.name || orgPhone) && (
          <Glass style={styles.card}>
            <Text style={styles.cardTitle}>Your organization</Text>
            {orgPhone ? (
              <Row c={{ label: profile?.organization?.name ?? profile?.college?.name ?? 'Organization', value: orgPhone, action: 'call' }} />
            ) : (
              <Text style={styles.muted}>
                {profile?.organization?.name ?? profile?.college?.name} — no phone number on file.
              </Text>
            )}
          </Glass>
        )}
